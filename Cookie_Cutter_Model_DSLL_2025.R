...
BRT_DSLL_Est_1<-gbm.step(CC_bite_DF, gbm.x = Preds, gbm.y = ncol(CC_bite_DF),tree.complexity=5,learning.rate = 0.005 , bag.fraction = 0.75,family="bernoulli", n.folds=5)
...
DSLL_CC_BRT_Models1<-fit.brt.n_eval_Balanced_Fixed(CC_bite_DF, gbm.x = Preds, gbm.y=ncol(CC_bite_DF) ,lr=0.005, tc=5,family="bernoulli",nt=Ntrees, bag.fraction = 0.75,50)

var_tested<-names(CC_bite_DF[,Preds])

PA_Model<-DSLL_CC_BRT_Models1[[1]]
...
DSLL_CC_BRT_Models<-fit.brt.n_eval_Balanced_Fixed(CC_bite_DF, gbm.x = Reduced_Predictors, gbm.y=ncol(CC_bite_DF) ,lr=0.005, tc=5,family="bernoulli",nt=Ntrees, bag.fraction = 0.75,50)

saveRDS(DSLL_CC_BRT_Models, "PA_DSLL_Cookie_Cutter_Bite_BRT_Models_2025.rds")



DSLL_CC_BRT_Models<-readRDS("PA_DSLL_Cookie_Cutter_Bite_BRT_Models_2025.rds")



Reef_Fish_BRT_Models<-DSLL_CC_BRT_Models
...
write.csv(Model_Skill_PA,"PA_Metrics_Cookie_Cutter_DSLL_Response_2025.csv")
...
for (j in 1:1){#length(Q)
  ...
  if (is.null(Model)==FALSE){
    ...
    DSLL_All_percent_contribution<-All_percent_contribution
    #
    #
    Mean_PA_Contributions<-as.data.frame(t(rowMeans(percent_contrib)))
    write.csv( Mean_PA_Contributions,paste0("Var_Contributions_",Species_Name,"_PA_DSLL_2025.csv"))
    ...
    png(paste0("Radar_Chart_",Species_Name,"_PA_DSLL_2025.png"), height=6, width=6, units="in",res=300)
    radarchart(PA_Predictors_Plot,  pfcol=rgb(0.0,0.3,0.5,0.5), pcol=rgb(0.0,0.3,0.5,0.5), title=paste0(Species_Name,"_PA"))
    dev.off()
    #
    All_percent_contribution<-cbind(rownames(percent_contrib), paste(round(rowMeans(percent_contrib),2), round(rowSds(percent_contrib),2), sep=" ± "))
    #
    png(paste0("Partial_plots_",Species_Name,"_PA_DSLL_2025.png"), height=18,width=14, res=300, units="in")
    par(mfrow=c(5,5))
    mn_part_plot<-list()
