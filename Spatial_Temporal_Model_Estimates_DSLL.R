...
Cookie_Cutter_DSLL_BRT_Models<-readRDS( "PA_DSLL_Cookie_Cutter_Bite_BRT_Models_2025.rds")
Cookie_Cutter_DSLL_Estimates<-matrix(,nrow=nrow(CC_bite_DF), ncol=50)
#df$vessel_name<-NA
#df$permit_num<-NA
for (i in 1:1){
  for (k in 1:50){
    Cookie_Cutter_DSLL_Estimates[,k]<-predict.gbm(Cookie_Cutter_DSLL_BRT_Models[[1]][[k]], CC_bite_DF,
                                         n.trees=Cookie_Cutter_DSLL_BRT_Models[[1]][[k]]$gbm.call$best.trees, type="response")
    print(paste("completed", k, "of 50"))}
}

#saveRDS(Cookie_Cutter_DSLL_Estimates, "CC_DSLL_Model_Estimate_2025.rds")

Cookie_Cutter_DSLL_Est<-readRDS("CC_DSLL_Model_Estimate_2025.rds")

Mean_Cookie_Cutter_DSLL<-rowMeans(Cookie_Cutter_DSLL_Est, na.rm=T)

df_predictions<-cbind(CC_bite_DF, Mean_Cookie_Cutter_DSLL)
df_predictions$Count<-1

Monthy_Estimates<-ddply(df_predictions, .(year_end, month), summarize, Real=mean(CC_Bite_PA, na.rm=T), Modeled=mean(Mean_Cookie_Cutter_DSLL, na.rm=T),Total_Sets=sum(Count), Mean_Night_Hrs=mean(night_hrs, na.rm=T), Mean_Fish=mean(Fish_Count, na.rm=T), Hooks=mean(HKS, na.rm=T))
...
png("~/Documents/Cookie_Cutter/Updated_Figures/CC_Bites_through_Time_DSLL_2025.png", height=5, width=10, units="in", res=300)
plot(Monthy_Estimates$Time, Monthy_Estimates$Real, type="l", lwd=3, ylim=c(0.4, 0.95), ylab=c("Prob/Prop Sets with Bite"), xlab="Time", cex.lab=1.25)
par(new=T)
plot(Monthy_Estimates$Time, Monthy_Estimates$Modeled, type="l",col="red", lty=3, lwd=3, ylim=c(0.4, 0.95), ylab=c("Prob/Prop Sets with Bite"), xlab="Time", cex.lab=1.25)
dev.off()




png("~/Documents/Cookie_Cutter/Updated_Figures/Monthly_Night_Hours.png", height=5, width=10, units="in", res=300)
plot(Monthy_Estimates$Time, Monthy_Estimates$Mean_Night_Hrs, type="l", lwd=3, xlab="Time", ylab="Mean Night Hours per Set", cex.lab=1.25)
dev.off()

png("~/Documents/Cookie_Cutter/Updated_Figures/Monthly_Fish_DSLL.png", height=5, width=10, units="in", res=300)
plot(Monthy_Estimates$Time, Monthy_Estimates$Mean_Fish, type="l", lwd=3, xlab="Time", ylab="Mean Fish Count per Set", cex.lab=1.25)
dev.off()


png("~/Documents/Cookie_Cutter/Updated_Figures/Monthly_Hooks_DSLL.png", height=5, width=10, units="in", res=300)
plot(Monthy_Estimates$Time, Monthy_Estimates$Hooks, type="l", lwd=3, xlab="Time", ylab="Mean Hooks per Set", cex.lab=1.25)
dev.off()
...
Binned_Est<-ddply(df_predictions, .(Lon, Lat), summarise, Real=mean(CC_Bite_PA, na.rm=T), Modeled=mean(Mean_Cookie_Cutter_DSLL, na.rm=T),Total_Sets=sum(Count), Mean_Night_Hrs=mean(night_hrs, na.rm=T), Unq_Captains=length(unique(permit_num)))
...
png("Modeled_DSLL_CC_Bite_Model_estimates_2025.png", res=300, height=6, width=8, units="in")
ggplot()+coord_fixed(ratio = 1)+geom_raster(data= Binned_Estimates,aes(x=Lon, y=Lat, fill=Modeled))+scale_fill_viridis_c( guide = guide_colourbar(title="Pr(Bite)"), limit=c(0.15,1))+
...
png("Modeled_DSLL_CC_Bite_Actual_Prop_2025.png", res=300, height=6, width=8, units="in")
ggplot()+coord_fixed(ratio = 1)+geom_raster(data= Binned_Estimates,aes(x=Lon, y=Lat, fill=Real))+scale_fill_viridis_c( guide = guide_colourbar(title="Prop. Bite"), limit=c(0.15,1))+
  geom_sf(data=world)+theme_bw()+geom_sf()+coord_sf(xlim=c(-178,-125), ylim=c(0, 40))+
  theme(legend.title=element_text(size=16),legend.text=element_text(size=14),legend.direction = "vertical", legend.box = "vertical")+ggtitle("")
dev.off()
